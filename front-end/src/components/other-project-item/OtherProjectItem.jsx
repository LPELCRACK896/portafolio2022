    ...
    <ReactCardFlip isFlipped = {isFliped} className='other-project-item-card'  >
      ...
      <div className='other-project-item-card' style={{border: `2px solid ${card_info.color}`,}}>
        ...
        <div className='tech-logos'>
          {card_info.details.technologies.map((tech, index) => <Link to={`/programm-languages/${tech}`} key={`${index}`}><img src={technologies.get(tech)} alt="tec" style={{height: '100%'}} className="selectable-logo-on-back-card"></img></Link>)}
        </div>
        <h4 className='other-titles-into-back-part-card-web'>Ver proyecto</h4>
        <div className='tech-logos'>
          {Object.keys(card_info.details.watchProject).map((platform, index) => <a key={index}target="_blank" href={card_info.details.watchProject[platform]}> <img src={platforms.get(platform)} alt="tec" style={{height: '100%'}} className="selectable-logo-on-back-card"></img></a>
